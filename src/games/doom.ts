...
export const doom: MiniGame3D = {
  ...
  start(ctx: MiniGame3DContext, opts: MiniGameOpts) {
     ...
     */
    // Масштаб от ВЫСОТЫ 720, но с оглядкой на ширину: портретному телефону
    // высоты хватает, а ширины нет — без второго ограничителя счётчик волны
    // наезжал на панель оружия, а ячейки статус-бара слипались. База 560
    // подобрана по самым широким элементам HUD; на ландшафтных экранах ширина
    // не лимитирует, и масштаб там прежний — строго height/720.
    // Пол 0.55 держит тач-мишени физически нажимаемыми на ландшафтном
    // телефоне (без него стики ужимались бы до ~46px радиуса).
    const hs = () => Math.max(0.55, Math.min(ctx.height / 720, ctx.width / 560));
    ...
    const drawHudInner = () => {
      ...
      if (phase === 'title') {
        ...
        g.fillStyle = '#7a5a50';
        // под последним пунктом: с шестью пунктами фиксированные 0.82H
        // попадали прямо на «РЕДАКТОР АРЕНИ»
        g.fillText(how, W / 2, Math.min(H * 0.94, H * 0.5 + menu.length * step + 26));
